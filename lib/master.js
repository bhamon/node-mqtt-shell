...
DESCRIPTION
  ...
  -C, --config=<CONFIG>
    Use a JSON-encoded configuration file for some options.
    Options in the configuration file take precedence over command line ones.
  -b, --port=<PORT>
...
if (args.config) {
  const raw = fs.readFileSync(args.config);
  const json = JSON.parse(raw);
  extend(true, args, json);
}
...
wsServer.on('connection', (_wsClient, _request) => {
  ...
  _wsClient.on('message', _msg => {
    ...
    if(_msg instanceof Buffer) {
      return
    }

    console.log(`publish ${topicTtyIn}`);
    mqttClient.publish(topicTtyIn, JSON.stringify(packet), {qos: 1});
  });
